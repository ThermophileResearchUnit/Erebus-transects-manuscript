...
env_lines <- ggarrange(temp_pH, temp_conductivity, temp_gwc, 
          temp_carbon, temp_nitrogen, temp_cn,
          ncol = 3, nrow = 2)
...
metals3<- envdata %>%
  select(Site, Temperature, Mg, P, Tl, Cr, Mn, Cd, S, Cu, Fe, Zn, Pb) 
...
metals_order <- c("Cr", "Mg", "P", "Cd", "Mn", "Tl","Cu", "Fe", "Pb", "S", "Zn")
...
plot_metal3_lines <- ggplot(metals3, aes(x = Temperature, y = values)) + 
...
```{r}
all_geochem <- ggarrange(env_box, env_lines,
                         plot_metal3_box, plot_metal3_lines,
          labels = c("A","B","C","D"),
          ncol = 2,
          nrow = 2,
          heights = c(0.9, 1.5),
...
#ggsave("Figure 2A-D.png", all_geochem, width = 240, height = 160, units = "mm", dpi = 300)

#ggsave("Figure 2A-D.svg", all_geochem, width = 240, height = 160, units = "mm", dpi = 300)
...
metals3_tr <- envdata %>%
  select(Transect, Temperature, Mg, P, Tl, Cr, Mn, Cd, S, Cu, Fe, Zn, Pb) %>%
  filter(., Transect != "WC")
...
plot_metal3_lines_tr <- ggplot(metals3_tr, aes(x = Temperature, y = values)) + 
...
```{r}
all_geochem_tr <- ggarrange(env_box_tr, env_lines_tr,
                         plot_metal3_box_tr, plot_metal3_lines_tr,
          labels = c("A","B","C","D"),
          ncol = 2,
          nrow = 2,
          heights = c(0.9, 1.5),
...
#ggsave("Figure 3A-D.png", all_geochem_tr, width = 240, height = 160, units = "mm", dpi = 300)

#ggsave("Figure 3A-D.svg", all_geochem_tr, width = 240, height = 160, units = "mm", dpi = 300)
...
#ggsave("Figure S3A-B.png", pcoa_both, width = 240, height = 90, units = "mm", dpi = 300)

#ggsave("Figure S3A-B.svg", pcoa_both, width = 240, height = 90, units = "mm", dpi = 300)
